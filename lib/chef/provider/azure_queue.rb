  ...
  class Provider
    class AzureQueue < Chef::Provisioning::AzureDriver::AzureProvider
      action :create do
		...
		Azure.config.sb_namespace = new_resource.options.namespace
		Azure.config.sb_access_key =  new_resource.options.access_key		
        Chef::Log.info("Creating AzureQueue: #{new_resource.name} in AzureServiceBus ${new_resource.options.namespace}")		
      ...
      action :destroy do
		...
		Azure.config.sb_namespace = new_resource.options.namespace
		Azure.config.sb_access_key =  new_resource.options.access_key
        Chef::Log.info("Destroying AzureQueue: #{new_resource.name} in AzureServiceBus ${new_resource.options.namespace}")
        sbs = Azure::ServiceBusService.new
